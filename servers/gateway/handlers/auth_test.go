...
func TestUsersHandler(t *testing.T) {
	var jsonStr = []byte(`{"Email": "test@test.com", "Password": "password123", "PasswordConf": "password123", "FirstName": "firstname", "LastName": "lastname"}`)
...
func TestSpecificUserHandler(t *testing.T) {
	...
	for _, c := range cases {
		...
		resp := httptest.NewRecorder()
		sessions.BeginSession(custCtx.SigningKey, custCtx.SessionStore, state, resp)
