...
func (c *Context) SpecificPlantsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		_, insertErr := c.PlantStore.Exec(insertQ, c.UserID, plantID)
		if insertErr != nil {
			http.Error(w, "Error inserting into databse", http.StatusInternalServerError)
			return
		}
...
func (c *Context) SpecificUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		_, insertErr := c.PlantStore.Exec(insertQ, c.UserID, plantID, qToAdd.Quanity)
		if insertErr != nil {
		...
		_, delErr := c.PlantStore.Exec(deleteQ, c.UserID, plantID)
		if delErr != nil {
...
func (c *Context) EmissionsHandler(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT UP.UserID, SUM(UP.Quantity * P.CO2PerUnit) as total FROM UserPlants UP JOIN Plants P on UP.PlantID = P.PlantID WHERE UP.UserID = ?"
	em := Emissions{}
